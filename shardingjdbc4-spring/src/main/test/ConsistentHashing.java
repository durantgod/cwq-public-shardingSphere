...
//            }
            long hash = hashFunction.hash(node);
            circle.put(hash, node);
        }
    }
...
//        // 初始化节点列表
//        List<String> nodes = Arrays.asList("ds0", "ds1", "ds2", "ds3");
//        // 创建一致性哈希实例，设定每个节点生成100个虚拟节点
//        ConsistentHashing consistentHashing = new ConsistentHashing(nodes, 8);
//        // 测试分配逻辑
//        for (int i = 0; i < 20; i++) {
//            System.out.println("i == " + i + " 所在节点为==> " + consistentHashing.getTargetNode(String.valueOf(i)));
//        }

        // 初始化表列表
        List<String> tables = Arrays.asList("t_ent_order_item_0", "t_ent_order_item_1", "t_ent_order_item_2", "t_ent_order_item_3", "t_ent_order_item_4", "t_ent_order_item_5", "t_ent_order_item_6", "t_ent_order_item_7");
        // 创建一致性哈希实例，设定每个节点生成100个虚拟节点
        ConsistentHashing tconsistentHashing = new ConsistentHashing(tables, 1);

        // 测试分表逻辑
        for (int i = 33; i < 67; i++) {
            System.out.println("i == " + i + " 所在表为==> " + tconsistentHashing.getTargetNode(String.valueOf(i)));
        }
    }
